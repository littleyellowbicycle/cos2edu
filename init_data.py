...
async def init_sample_material():
    async with AsyncSessionLocal() as db:
        try:
            result = await db.execute(select(Material).limit(1))
            existing = result.scalar_one_or_none()
            if existing:
                logger.info("教材已存在，跳过初始化")
                return
...
这就是卷积神经网络的基本概念。理解这些概念后，你就能更好地理解CNN是如何"看"图片的了。"""
            ...
            await db.commit()
            logger.info("已初始化示例教材")
        except Exception as e:
            logger.error(f"初始化教材失败: {e}")
            await db.rollback()
...
async def main():
    ...
    logger.info("正在初始化示例教材...")
    await init_sample_material()
    
    logger.info("初始化完成！")
    logger.info("\n使用方法：")
    logger.info("1. 安装依赖: pip install -r requirements.txt")
    logger.info("2. 启动服务: python main.py")
    logger.info("3. 访问 http://localhost:8000")
